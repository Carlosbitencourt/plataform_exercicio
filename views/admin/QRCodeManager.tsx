...
import { QrCode, Copy, Download, Share2, Calendar, ShieldCheck, Send, MessageCircle, RefreshCw } from 'lucide-react';
import { getDB, addQRCode, getLocalDate } from '../../services/storage';
...
const QRCodeManager: React.FC = () => {
  ...
  const loadActiveQR = () => {
    const db = getDB();
    const qr = db.qrCodes.find(q => q.date === today && q.active);
    setActiveQR(qr || null);
  };
